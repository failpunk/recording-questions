...
class sfPHPOpenID {
  ...
  private $request_fields_AX = array('fullname' => 'http://axschema.org/namePerson', 'email' =>'http://axschema.org/contact/email');
  private $required_AX_fields = array('fullname', 'email'); // List of required fields from $request_fields_AX. Default (= not specified) is not required.
   ...
   */
  public function getRedirectURL($immediate = false)
  {
    ...
    var_dump($this->request_fields_AX);
    if ($ax_request) {
      foreach ($this->request_fields_AX as $alias => $url) {
        ...
        echo $url.'__'.$this->getCountForAXField($alias).'__'.$this->isRequiredAXField($alias).'__'.$alias;
      }
    ...
    if ($auth_request->shouldSendRedirect()) {
        ...
        $form_html = $auth_request->htmlMarkup($this->getTrustRoot(), $this->getApprovedURL(),
  ...
  public function setRequestFields($fields)
  {
    foreach ($fields as $id => $field) {
      if (!empty($field)) {
        if ( in_array($field, $available_sreg_values) && !in_array($field, $this->request_fields_sreg)) {
          $this->request_fields_sreg[] = $field;
          $this->request_fields_AX[$mapping_sreg_ax[$field]] = $field;
        }
