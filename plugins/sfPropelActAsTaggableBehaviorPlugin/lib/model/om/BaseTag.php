...
abstract class BaseTag extends BaseObject  implements Persistent {
  ...
  const PEER = 'TagPeer';
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Tag has previously been saved, it will retrieve
	 * related UserTags from storage. If this Tag is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collUserTags === null) {
			if ($this->isNew()) {
			   $this->collUserTags = array();
				...
				$criteria->add(UserTagPeer::TAG_ID, $this->id);
	 ...
	 */
	public function getUserTagsJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(TagPeer::DATABASE_NAME);
		}
		...
		if ($this->collUserTags === null) {
			if ($this->isNew()) {
				...
				$this->collUserTags = UserTagPeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
